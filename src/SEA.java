...
public class SEA{	
	...
	public static void extract(String filePath, CSVPrinter printer) throws IOException {
        ...
        for(int i = 0; i < arr.size() && count <= 5; i++) 
        	if(arr.get(i).trim().equals("X") && (arr.get(i-1).trim().equals("Unk") || NumberUtils.isNumber(arr.get(i-1).trim()))) {
        		if(arr.get(i-1).trim().equals("Unk")) scores[count] = 0;
        		else scores[count] = Integer.parseInt(arr.get(i-1));
        		count++;
        	}
        ...
        // SEA forms also have a table but they are not always filled out, if they are we can also use the following method of reading off the table for scores
        /*scores[0] = Integer.parseInt(text.substring(text.indexOf("Human Health  (Y / N )") + 23, text.indexOf("/15", text.indexOf("Human Health  (Y / N )"))).trim());
        scores[1] = Integer.parseInt(text.substring(text.indexOf("/15", text.indexOf("Human Health  (Y / N )")) + 3, text.indexOf("/-15", text.indexOf("Human Health  (Y / N )"))).trim());

        scores[6] = Integer.parseInt(text.substring(text.indexOf("Cultural  (Y / N )") + 19, text.indexOf("/15", text.indexOf("Cultural  (Y / N )"))).trim());
        scores[7] = Integer.parseInt(text.substring(text.indexOf("/15", text.indexOf("Cultural  (Y / N )")) + 3, text.indexOf("/-15", text.indexOf("Cultural  (Y / N )"))).trim());

        double pos = Integer.parseInt(text.substring(text.indexOf("Economic (Y / N )") + 18, text.indexOf("/70", text.indexOf("Economic (Y / N )"))).trim());
        double neg = Integer.parseInt(text.substring(text.indexOf("/70", text.indexOf("Economic (Y / N )")) + 3, text.indexOf("/-70", text.indexOf("Economic (Y / N )"))).trim());
        pos = pos/70.0; neg = neg/70.0;*/        
