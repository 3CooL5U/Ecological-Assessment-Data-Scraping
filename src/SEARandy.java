...
public class SEARandy {
	...
	public static void main(String args[]) throws IOException {
		...
		String[] HEADERS = { "scientificName", "Other scientific names", "stateCommonName", "Hum_health_pos",
				"Hum_health_neg", "Ind1_ag_pos", "Ind2_for_pos", "Ind1_ag_neg", "Ind2_for_neg", "Soc_rec_pos",
				"Soc_rec_neg" };

		// headers for check()
		// String[] HEADERS = {"scientificName", "Other scientific names",
		// "stateCommonName", "type", "sign", "word", "line"};
	...
	public static void extract(String filePath, CSVPrinter printer) throws IOException {
		...
		// create an arraylist of possible choices in the assessment so we can find
		// repeats for the score
		 ...
		 * Forms filled out by Randy G. Westbrooks use this algorithm, each type uses a
		 * different one
		 */
		int q = 0;
		for (int i = 1; i < arr.size() && q < 6; i++)
			if (arr.get(i).trim().equals(arr.get(i - 1).trim())) {
				if (arr.get(i).trim().equals("Unk")) {
					scores[q] = 0;
					q++;
				} else if (NumberUtils.isNumber(arr.get(i).trim())
						&& num.contains((int) (Double.parseDouble(arr.get(i).trim())))) {
		...
		// likewise for negative impacts on agriculture, and both pos and neg impacts on
		// forestry
		for (int i = 0; i < agList.length; i++)
			if (negDiscussion.contains(agList[i])) {
				scores[4] = neg;
